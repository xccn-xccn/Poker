...
def find_or_create_game():
    ...
    for room_name, game_table in active_games.items():
        ...
        taken_seats = [p.get('seat') for p in game_table.players if p.get('is_human_player')]
...
def handle_disconnect():
    ...
    if player_id in player_to_room:
        ...
        if room_name in active_games:
            ...
            emit('game_update', get_state(game_table, 0), room=room_name) # 0 is a default seat
...
def handle_start_hand(data):
    """Client clicked 'Deal'."""
    player_id = request.sid
    if player_id not in player_to_room: return # Only players can start
...
def handle_player_action(data):
    ...
    player_id = request.sid
    if player_id not in player_to_room: return # Only players can act
    ...
    if seat != game_table.current_player_i:
        print(f"Action from player {seat} received, but it's player {game_table.current_player_i}'s turn.")
